...
const projects = [
  {
    ...
    tech: ["Mediasoup", "WebRTC", "Next.js", "Node.js", "Docker", "Terraform", "AWS"],
  ...
  },
  {
    ...
    tech: ["PyTorch", "Transformers", "Llama3", "BERT", "RoBERTa", "Scikit-Learn", "DeepSpeed"],
...
export default function Projects() {
  return (
    <div className="max-w-6xl mx-auto px-6 py-24">
      ...
      >
        <h1 className="text-4xl md:text-5xl font-bold mb-16">Selected <span className="text-muted-foreground">Projects</span></h1>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project, index) => {
            const mainLink = project.live !== "#" ? project.live : project.github;
            ...
            return (
              ...
              >
                {isClickable ? (
                  <Link href={mainLink} target="_blank" rel="noopener noreferrer" className="block focus:outline-none cursor-pointer group/link relative z-10">
                    <div className="h-48 overflow-hidden relative">
                      ...
                      <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover/link:opacity-100 transition-all duration-300 bg-black/40 backdrop-blur-[2px]">
                        <div className="bg-primary text-black px-4 py-2 rounded-full flex items-center gap-2 font-bold transform translate-y-4 group-hover/link:translate-y-0 shadow-[0_0_20px_rgba(251,146,60,0.4)] transition-transform duration-300">
                          {isGithubLink ? (
                            <><FaGithub size={18} /> <span className="text-sm">Visit Github</span></>
                          ) : (
                            <><ExternalLink size={18} /> <span className="text-sm">Visit Project</span></>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="px-6 pt-6 relative z-20 -mt-12 pointer-events-none">
                  <h3 className="text-2xl font-bold text-white mb-3 pointer-events-auto">{project.title}</h3>
                  <p className="text-muted-foreground mb-6 line-clamp-4 pointer-events-auto">{project.desc}</p>
                </div>
                ...
                <div className="px-6 pb-6 relative z-20 flex-grow flex flex-col justify-end">
                  ...
                  <div className="flex items-center gap-4 pt-4 border-t border-white/10">
                    <Link href={project.github} className="text-muted-foreground hover:text-white transition-colors flex items-center gap-2 text-sm">
                      <FaGithub size={18} />
                      Code
                    </Link>
                    {project.live !== "#" && (
                      <Link href={project.live} className="text-muted-foreground hover:text-white transition-colors flex items-center gap-2 text-sm">
